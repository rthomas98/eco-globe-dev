...
function StepLayout({
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      ...
      <div className="flex flex-1 justify-center overflow-y-auto px-4 py-6 sm:px-6 sm:py-8">
...
function Notice({
  ...
  return (
    <p className={`rounded-lg px-4 py-2 text-sm ${styles[tone]}`}>{children}</p>
  );
}
...
export function AddListingPage() {
  ...
  const sellerCompany = useMemo(() => {
    ...
    return (
      ...
      companies.find(
        (c) => c.companyTypeCode === "seller" || c.companyTypeCode === "both",
      )
    );
  }, [user]);
  ...
  useEffect(() => {
    ...
    if (!draft) {
      setNotice({
        tone: "error",
        text: "That local draft is no longer on this device.",
      });
      return;
    }
    ...
    setNotice({
      tone: "info",
      text: `Resumed "${draft.title}" from this device. It is not on EcoGlobe until you save it.`,
    });
  }, [draftParam]);

  const up = <K extends keyof ListingForm>(k: K, v: ListingForm[K]) =>
    setForm((p) => ({ ...p, [k]: v }));
  const toggleClaim = (c: string) =>
    up(
      "claims",
      form.claims.includes(c)
        ? form.claims.filter((x) => x !== c)
        : [...form.claims, c],
    );
  const addSpecRow = () =>
  ...
  const updateSpecRow = (i: number, key: "label" | "value", v: string) =>
    up(
      "additionalSpecs",
      form.additionalSpecs.map((s, idx) =>
        idx === i ? { ...s, [key]: v } : s,
      ),
    );
  ...
  const saveInFlight = useRef<Promise<BackendListing | null> | null>(null);
  const savedRecord = useRef<BackendListing | null>(null);
  const persistDraft = async (statusCode?: "draft" | "pending_review") => {
    const issues = validateDraft(form);
    if (issues.length > 0) {
      setNotice({
        tone: "error",
        text: issues.map((i) => i.message).join(" "),
      });
      return null;
    }
    if (!companyId) {
      setNotice({
        tone: "error",
        text: "Complete seller onboarding before saving a listing.",
      });
      return null;
    }
    setBusy(true);
    ...
    try {
      const body = formToWriteBody(form, {
        sellerCompanyId: savedRecord.current ? undefined : companyId,
        listingStatusCode: statusCode,
      });
      const saved = savedRecord.current
        ? await updateListing(savedRecord.current.id, body)
  ...
  const ensureListing = async () => {
    if (savedRecord.current) return savedRecord.current.id;
    const issues = validateDraft(form);
    if (issues.length)
      throw new Error(issues.map((issue) => issue.message).join(" "));
    if (!saveInFlight.current) saveInFlight.current = persistDraft("draft");
    const saved = await saveInFlight.current;
    saveInFlight.current = null;
    if (!saved)
      throw new Error(
        "The draft could not be saved. Check the form message and retry the upload.",
      );
    return saved.id;
  };
  const handleSaveDraft = async () => {
    ...
    if (saved) {
      setNotice({
        tone: "success",
        text: `Draft saved to EcoGlobe as listing #${saved.id}. You can keep editing or return later.`,
      });
    }
  };
  ...
  const moqLabel = formatQuantityWithUnitName(
    parseOptionalNumber(form.moq),
    form.unit,
  );
  const qtyLabel = formatQuantityWithUnitName(
    parseOptionalNumber(form.qty),
    form.unit,
  );
  ...
  const specs: Array<[string, string]> = [
    ...
    [
      "Material type",
      MATERIAL_TYPE_OPTIONS.find((m) => m.value === form.materialTypeCode)
        ?.label ?? "",
    ],
        ...
        >
          ...
          <div className="flex flex-col gap-6">
            {sellerCompany ? (
              ...
              <Notice tone="error">
                No active seller company found on your session. Complete seller
                onboarding first.
              </Notice>
            )}
            <Input
            ...
            <Select
              label="Material type (marketplace classification)"
              id="material-type"
              options={MATERIAL_TYPE_OPTIONS}
              value={form.materialTypeCode}
              onChange={(e) => up("materialTypeCode", e.target.value)}
            />
            <div>
            ...
            <ListingDocumentUploader
              ...
              hint={
                record
                  ? undefined
                  : "Your draft is saved automatically before uploading photos. Accepts PNG, JPEG or WebP up to 5 MB each."
              }
            />
          </div>
        </StepLayout>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div>
              ...
              <p className="mb-3 text-xs text-neutral-500">
                Add anything specific to this feedstock — sulfur %, moisture %,
                pellet dimension, etc.
              </p>
                ...
                {form.additionalSpecs.map((spec, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <input
                      ...
                      value={spec.label}
                      onChange={(e) =>
                        updateSpecRow(i, "label", e.target.value)
                      }
            ...
            <details className="rounded-lg bg-neutral-50 p-4">
              <summary className="cursor-pointer text-sm font-medium text-neutral-700">
                Optional details (Listing type, Grade, Color, Shelf life,
                Storage, Package, Weight, Usage, Place of origin)
              </summary>
              <div className="mt-4 flex flex-col gap-4">
                ...
                <Select
                  ...
                  onChange={(e) => up("listingType", e.target.value)}
                />
                <Select
                  label="Grade / purity"
                  id="gr"
                  ...
                  value={form.grade}
                  onChange={(e) => up("grade", e.target.value)}
                />
                <Input
                  label="Color"
                  id="color"
                ...
                <Input
                  ...
                  onChange={(e) => up("shelfLife", e.target.value)}
                />
                <Input
                  label="Storage & handling"
                  id="sh"
                  value={form.storage}
                  onChange={(e) => up("storage", e.target.value)}
                />
                <Input
                  label="Package"
                  id="pkg"
                  value={form.pkg}
                  onChange={(e) => up("pkg", e.target.value)}
                />
                <Input
                  label="Weight"
                  id="wt"
          ...
          <div className="flex flex-col gap-6">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Select
                label="Quantity unit"
                id="unit"
                options={unitOptionsFor(form.unit)}
                value={form.unit}
              ...
              <Select
                ...
                onChange={(e) => up("currencyCode", e.target.value)}
              />
            </div>
            <div>
              <label
                ...
                className="mb-1.5 block text-sm font-medium text-neutral-900"
              >
                Unit price ({form.currencyCode || "currency"} per{" "}
                {form.unit ? unit.singular : "unit"})
              </label>
              <input
                id="price"
                type="text"
              ...
              <p className="mt-1 text-xs text-neutral-500">
                Blank means no price yet; enter 0 only if the material is
            ...
            <div>
              <label
                ...
                className="mb-1.5 block text-sm font-medium text-neutral-900"
              >
                Minimum Order Quantity (MOQ) in{" "}
                {form.unit ? unit.plural : "the chosen unit"}
              </label>
              <div className="flex">
                <input
                  id="moq"
                  ...
                  inputMode="decimal"
                  value={form.moq}
                  onChange={(e) => up("moq", e.target.value)}
                  className="flex-1 rounded-l-lg px-4 py-3 text-sm outline-none placeholder:text-neutral-400"
                  style={{ border: "1px solid #E0E0E0", borderRight: "none" }}
                />
                <span
                  className="rounded-r-lg bg-neutral-50 px-3 py-3 text-sm text-neutral-700"
                  style={{ border: "1px solid #E0E0E0" }}
                >
                  {form.unit ? unit.short : "—"}
                </span>
              </div>
            </div>
            <div>
              ...
              {(() => {
                ...
                return q !== null && m !== null && m > q ? (
                  <p className="mt-1 text-xs text-red-700">
                    MOQ cannot exceed the available quantity.
                  </p>
                ) : null;
              })()}
            </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            >
              ...
              <p className="mb-3 text-xs text-neutral-700">
                Buyers cannot purchase this feedstock until a Safety Data Sheet
                PDF is uploaded (EU REACH or equivalent local format).
              </p>
              <ListingDocumentUploader
                listingId={record?.id ?? null}
                ensureListing={ensureListing}
                ...
                hint={
                  record
                    ...
                    : "Your draft is saved automatically before uploading the SDS PDF (up to 5 MB)."
                }
              />
              {hasSds && (
                <p className="mt-2 flex items-center gap-1 text-xs text-green-700">
                  <CheckCircle2 className="size-3" /> SDS on file for this
                  listing.
                </p>
              )}
            </div>
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {CLAIM_OPTIONS.map((c) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={form.claims.includes(c)}
              ...
              {form.customClaims.length > 0 && (
                <ul className="mt-3 flex flex-wrap gap-2">
                  {form.customClaims.map((claim) => (
                    <li
                      key={claim}
                      className="flex items-center gap-1 rounded-full bg-neutral-100 px-3 py-1 text-xs text-neutral-800"
                    >
                      {claim}
                      <button
                        type="button"
                        ...
                        onClick={() =>
                          up(
                            "customClaims",
                            form.customClaims.filter((c) => c !== claim),
                          )
                        }
              ...
              {form.claims.includes("Others") && (
                <div className="mt-3">
                  <Input
                    label="Describe the other claim"
                    id="other-claim"
                    value={form.otherClaim}
                    onChange={(e) => up("otherClaim", e.target.value)}
                  />
                </div>
              )}
            </div>
            <ListingDocumentUploader
              ...
              hint={
                record
                  ? "Accepts PDF up to 5 MB each."
                  : "Your draft is saved automatically before uploading certification PDFs (up to 5 MB each)."
              }
            />
            <div
            ...
            >
              ...
              <p className="mb-3 text-xs text-neutral-700">
                Buyers see these on the product page. PDF up to 5 MB each.
              </p>
              <div className="flex flex-col gap-4">
                <ListingDocumentUploader
                  ...
                  hint={
                    record
                      ? "Accepts PDF up to 5 MB."
                      : "Your draft is saved automatically before uploading the TDS (PDF up to 5 MB)."
                  }
                />
                <ListingDocumentUploader
                  listingId={record?.id ?? null}
                  ...
                  hint={
                    record
                      ? "Accepts PDF up to 5 MB."
                      : "Your draft is saved automatically before uploading the COA (PDF up to 5 MB)."
                  }
                />
              </div>
            </div>
            <TextArea
              id="sustain-notes"
              label="Sustainability Notes (Optional)"
              rows={4}
              value={form.sustainNotes}
              onChange={(v) => up("sustainNotes", v)}
            />
          </div>
        </StepLayout>
      )}
      ...
      {step === 6 && (
        ...
        >
          ...
          <p className="mb-6 text-sm text-neutral-600">
            Confirm the facility this listing ships from. Facilities are saved
          ...
          <div className="flex flex-col gap-5">
            ...
            />
            <Input
              label="Origin note (optional)"
              id="ol"
              placeholder="e.g. Gate 4, north yard"
      ...
      {step === 7 && (
        <div className="flex min-h-screen flex-col bg-white">
            ...
            <div className="min-w-0 flex-1 px-4 py-6 sm:px-6 sm:py-8">
              <div className="mx-auto max-w-[900px]">
                <p className="mb-4 rounded-lg bg-neutral-50 px-4 py-2 text-xs text-neutral-600">
                  This preview shows exactly what you entered
                  {record ? ` and what is saved as listing #${record.id}` : ""}.
                ...
                <p className="mb-6 flex flex-wrap items-center gap-2 text-sm text-neutral-500">
                  <span>
                    {selectedLocation
                      ? formatCompanyLocation(selectedLocation)
                      : "No facility selected"}
                  </span>
                  <span className="rounded bg-neutral-100 px-2 py-0.5 text-xs">
                    MOQ: {moqLabel ?? "not set"}
                  </span>
                  <span className="rounded bg-neutral-100 px-2 py-0.5 text-xs">
                    Carbon data: not provided
                  </span>
                </p>
                    ...
                    <div className="relative mb-4 flex h-[380px] items-center justify-center overflow-hidden rounded-xl bg-neutral-100">
                      {previewListing?.image ? (
                        ...
                        <p className="px-6 text-center text-sm text-neutral-500">
                          No photos uploaded yet. Add photos in step 1 after
                          saving the draft.
                        </p>
                      )}
                    </div>
                    {photos.length > 1 && (
                      <div className="flex gap-2 overflow-x-auto">
                        {previewListing?.images.map((img, i) => (
                          <div
                            key={img}
                            className={`size-16 shrink-0 overflow-hidden rounded-lg ${i === 0 ? "ring-2 ring-neutral-900" : ""}`}
                          >
                            <img
                              src={img}
                              alt=""
                  ...
                  <div className="w-full shrink-0 lg:w-[300px]">
                    ...
                    >
                      ...
                      <p className="mb-4 text-sm text-neutral-500">
                        Minimum Order Quantity (MOQ): {moqLabel ?? "not set"}
                      </p>
                      <p className="mb-2 text-sm text-neutral-700">
                        Available: {qtyLabel ?? "not set"}
                      </p>
                      <p className="mb-4 text-xs text-neutral-500">
                        Shipping is quoted per order and is not included in the
                        listed price.
                      </p>
                      <Button
                        variant="primary"
                    ...
                    {!hasSds && (
                      <p className="mt-3 flex items-start gap-2 rounded-md bg-amber-50 px-3 py-2 text-xs text-amber-700">
                        <AlertTriangle className="mt-0.5 size-3 shrink-0" />
                        No SDS uploaded: buyers will not be able to purchase
                        until you add it.
                      </p>
                    )}
                  </div>
                    ...
                    {form.claims.length > 0 && (
                      <p className="text-sm text-neutral-700">
                        {form.claims
                          .filter((c) => c !== "Others")
                          .concat(
                            form.claims.includes("Others") && form.otherClaim
                              ? [form.otherClaim]
                              : [],
                          )
                          .join(" · ")}
                ...
                {documents.length === 0 ? (
                  ...
                  <ul className="flex flex-col gap-1 text-sm text-neutral-700">
                    {documents.map((d) => (
                      <li key={d.id}>
                        {d.documentTypeCode.toUpperCase()}: {d.fileName}
                      </li>
                    ))}
                  </ul>
                )}
                ...
                <div className="mb-4 flex items-center gap-3">
                  <div className="flex size-10 items-center justify-center rounded-full bg-neutral-200 text-sm font-semibold text-neutral-600">
                    {(sellerCompany?.legalName ?? "?")
                      .slice(0, 1)
                      .toUpperCase()}
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-neutral-900">
                      {sellerCompany?.legalName ?? "Unknown company"}
                ...
                >
                  <p className="text-sm text-neutral-700">
                    Buyers can run a transportation-emissions estimate against
                    this listing. As the seller you can also estimate the value
                    recovered by selling instead of disposing.
                  </p>
                  {previewListing ? (
                    <div className="mt-4 flex flex-wrap gap-3">
                      ...
                      <CarbonCalculatorButton
                        ...
                        portal="seller"
                        variant="primary"
                        label="Estimate value recovery"
                        startAt="value-recovery"
                      />
                    </div>
                  ) : (
                    <p className="mt-3 text-xs text-neutral-500">
                      Save the draft to enable the calculators for this listing.
                    </p>
                  )}
                </div>
              </div>
          ...
          <div className="relative">
            ...
            {noticeNode && (
              <div className="px-4 pt-3 sm:px-6">{noticeNode}</div>
            )}
            <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-6">
              <Button
                variant="secondary"
