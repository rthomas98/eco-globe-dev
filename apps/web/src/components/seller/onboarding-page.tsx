  ...
  useEffect(() => {
    ...
    fetchOnboardingState()
      .then((state) => {
        ...
        // A returning seller keeps their recorded licence tier; the payload
        // always carries licenceTier, so without this prefill a retry would
        // silently downgrade a paid tier to the "free" default.
        const savedTier = state.sellerProfile?.licenceTierCode;
        if (savedTier) setLicenceTier(savedTier);
      })
      .catch(() => {
