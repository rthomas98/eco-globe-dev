...
export function LiveSignatureWorkspace({ role }: { role: WorkspaceRole }) {
  ...
  const selectedSignatures = useMemo(
    () =>
      signatures.filter(
        (signature) => signature.contractId === selectedContractId,
      ),
  ...
  const buyerSigners = selectedContract
    ? selectedSignatures.filter(
        (signature) =>
          signature.signerCompanyId === selectedContract.buyerCompanyId,
      )
    : [];
  const sellerSigners = selectedContract
    ? selectedSignatures.filter(
        (signature) =>
          signature.signerCompanyId === selectedContract.sellerCompanyId,
      )
    : [];
  const signersReady =
    buyerSigners.length === 1 &&
    sellerSigners.length === 1 &&
    selectedSignatures.every(
      (signature) => signature.signatureStatusCode === "not_sent",
    );
  ...
  async function assignSigner() {
    ...
    setError(undefined);
    try {
      await assignSelfAsSigner(selectedContract.id);
      setNotice(
        "You are registered as your company's signer. The other company must register its signer before the envelope can be sent.",
      );
      await refresh();
    } catch (cause) {
      setError(
        cause instanceof Error ? cause.message : "Unable to register signer.",
      );
    } finally {
      setBusy(undefined);
    }
  ...
  async function openSigning() {
    ...
    try {
      ...
      setError(
        cause instanceof Error ? cause.message : "Unable to open DocuSign.",
      );
      setBusy(undefined);
    }
  }
    ...
    <div>
      <div className="mb-6">
        ...
        <h1 className="mt-2 text-3xl font-bold text-neutral-900">
          Send, sign, and track agreements securely.
        </h1>
        <p className="mt-2 max-w-3xl text-sm text-neutral-600">
          DocuSign handles the signing ceremony. EcoGlobe updates the contract
          only after a verified DocuSign event and retains the completed
      ...
      {loading ? (
        <div className="rounded-2xl bg-white p-8 text-sm text-neutral-500">
          Loading contracts and signers…
        </div>
      ) : contracts.length === 0 ? (
        <div
        ...
        >
          ...
          <h2 className="mt-3 text-lg font-bold text-neutral-900">
            No contracts are ready for signature
          </h2>
          <p className="mt-2 text-sm text-neutral-500">
            Create the contract and assign one buyer and one seller signer
            first.
          </p>
        </div>
      ) : (
              ...
              {contracts.map((contract) => (
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-neutral-500">
                    {contract.buyerCompanyName} ↔ {contract.sellerCompanyName}
                  </p>
                  <p className="mt-2 text-xs font-semibold uppercase tracking-wide text-neutral-600">
                    {contract.contractStatusCode.replaceAll("_", " ")}
                  </p>
                </button>
              ))}
            </div>
          ...
          {selectedContract && (
            ...
            >
              ...
              {!providerReady && (
                ...
                >
                  DocuSign is not connected yet. You can prepare signer
                  assignments now. EcoGlobe must connect DocuSign before an
                  envelope can be sent or signed.
                </p>
              )}
              <div className="mt-5 space-y-3">
                {selectedSignatures.length === 0 && (
                  <p className="rounded-xl bg-amber-50 p-4 text-sm text-amber-800">
                    Assign one buyer signer and one seller signer before sending
                    this contract.
                  </p>
                )}
                {selectedSignatures.map((signature) => (
                  <div
                    key={signature.id}
                    className="flex items-center justify-between gap-3 rounded-xl bg-neutral-50 p-4"
                  >
                    <div>
                      ...
                      <p className="mt-1 text-xs text-neutral-500">
                        {signature.signerCompanyName}
                      </p>
                    </div>
                    <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-neutral-700">
                      {statusLabels[signature.signatureStatusCode] ||
                        signature.signatureStatusCode}
                    </span>
                  </div>
                ))}
              </div>
              ...
              {!selectedContract.providerEnvelopeId &&
                ...
                !selectedSignatures.some(
                  (s) => s.signerCompanyId === user?.activeCompanyId,
                ) && (
                  <div className="mt-4 rounded-xl border p-4">
                    <p className="mb-3 text-sm">
                      Each company must register its own signer. This step
                      prepares the request; it does not sign the agreement or
                      send an email.
                    </p>
                    <Button
                      disabled={Boolean(busy)}
                      onClick={() => void assignSigner()}
                    >
                      {busy === "assign"
                        ? "Saving…"
                        : "Register me as company signer"}
                    </Button>
                  </div>
                )}
              <p className="mt-4 text-sm text-neutral-600">
                {mySignature?.signatureStatusCode === "signed"
                  ? "Your signature is complete."
                  : !mySignature
                    ? "Signing is available only to the assigned signer for your company."
                    : !mySignature.providerEnvelopeId
                      ? "Once both signers are registered, send the envelope to enable signing."
                      : "Open DocuSign to review the agreement and sign."}
              </p>
                  ...
                  <div className="mt-5 rounded-xl border border-green-200 bg-green-50 p-4">
                    ...
                    <div className="mt-2 flex gap-4 text-sm">
                      <a
                        ...
                        href={`/api/backend/api/contracts/${selectedContract.id}/docusign-documents/agreement`}
                        target="_blank"
                        rel="noreferrer"
                      >
                        Signed agreement
                      </a>
                      {selectedContract.completionCertificateUrl && (
                        <a
                          className="font-semibold text-green-800 underline"
                          href={`/api/backend/api/contracts/${selectedContract.id}/docusign-documents/certificate`}
                          target="_blank"
                          rel="noreferrer"
                        >
                          Certificate
