...
export function AuthLayout({
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-neutral-100 px-4">
      {/* Logo */}
      <Link href="/" className="absolute left-[30px] top-[30px]">
      ...
      <Link
        href="/"
        className="absolute right-[30px] top-[30px] flex size-10 items-center justify-center rounded-full bg-white transition-colors hover:bg-neutral-200"
      >
      ...
      {/* Card */}
      <div
        className={`${cardWidth} w-full overflow-hidden rounded-xl bg-white`}
        style={{ boxShadow: "0 16px 40px -8px rgba(0,0,0,0.1)" }}
      >
        <div className="flex">
          {/* Form content */}
          <div className={`flex-1 ${showImage ? "p-[60px]" : "p-[60px]"}`}>
            {children}
          </div>
