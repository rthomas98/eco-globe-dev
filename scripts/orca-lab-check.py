#!/usr/bin/env python3
"""Run lab referral integration checks only against this worktree's owned SQL."""
import json
import runpy
...
with rt.lock(local.directory / "lock", blocking=False):
    with rt.lock(runtime.directory / "api.lock", blocking=False):
        ...
        code = rt.foreground(
            ["pnpm", "exec", "tsx", str(root / "scripts/orca-lab-integration.mjs")],
            root / "packages/backend",
            env,
        )
        print(json.dumps({"labIntegrationExitCode": code, "database": value["database"]}))
